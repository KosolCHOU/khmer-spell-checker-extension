...
function applyLang(lang) {
  ...
  selectElements.forEach(select => {
    const options = select.querySelectorAll('option[data-lang-en][data-lang-km]')
    options.forEach(option => {
      const text = lang === 'en' ? option.dataset.langEn : option.dataset.langKm
      if (text) {
        option.textContent = text
      }
    })
    // Apply Khmer font to select if in Khmer mode
    if (lang === 'km') {
      select.style.fontFamily = "'Khmer OS Battambang', 'Battambang', 'Noto Sans Khmer', serif"
    } else {
      select.style.fontFamily = "'Merriweather', 'Georgia', serif"
    }
  })
